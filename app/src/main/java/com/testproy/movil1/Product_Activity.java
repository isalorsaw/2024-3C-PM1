...
/*
    ...
    private void llenarCampos(int index)
    {
        ...
        try
        {
            ...
            ResultSet rs=s1.executeQuery("select * from tbl_producto where user_id='"+idselected+"'");
            while(rs.next())
            {
    ...
    {
        list=BDManager.getList("select producto_nombre from tbl_producto order by producto_nombre");
        listid=BDManager.getList("select producto_id from tbl_user order by producto_nombre");
